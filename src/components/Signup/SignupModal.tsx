...
function SignupModal() {
  ...
  const PassWordDoubleChecking = (
    password: string,
    pwDoubleCheck: string | undefined
  ) => {
  ...
  return (
    <Layout>
      ...
      {isNullForm.passwordVail ? (
        <div>* 비밀번호 확인 : 필수정보입니다!</div>
      ) : (
        <div>{passwordVal ? "" : "* 비밀번호 확인 : 동일하지 않습니다!"}</div>
      )}
      <IdDiv>
        <LoginInput
          type="text"
      ...
      <ChoiceLayout>
        <DrinkChoiceButton
          choice={form.enjoyDrink}
          onClick={() => setForm({ ...form, enjoyDrink: true })}
        >
          음주
        </DrinkChoiceButton>
        <DrinkChoiceButton
          choice={!form.enjoyDrink}
          onClick={() => setForm({ ...form, enjoyDrink: false })}
        >
          금주
        </DrinkChoiceButton>
      </ChoiceLayout>
      <ChoiceLayout>
        <AlcoholSelect
          value={form.favorLiquor}
          onChange={(e) => setForm({ ...form, favorLiquor: e.target.value })}
        >
